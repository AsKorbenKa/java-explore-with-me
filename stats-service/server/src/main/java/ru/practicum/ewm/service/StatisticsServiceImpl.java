    ...
    public List<ViewStats> getStats(LocalDateTime start, LocalDateTime end, List<String> uris, boolean unique) {
        ...
        if (uris.isEmpty()) {
            if (unique) {
                ...
                return repository.findDistinctIpByTimestampAfterAndTimestampBefore(start, end);
            } else {
                log.info("Получаем все данные статистики за определенное время.");
                return repository.findAllByTimestampAfterAndTimestampBefore(start, end);
            }
        } else {
            if (unique) {
                log.info("Получаем данные статистики по uri за определенное время c уникальными значениями ip.");
                return repository.findDistinctIpByUriAndTimestampAfterAndTimestampBefore(uris, start, end);
            } else {
                log.info("Получаем все данные статистики по uri за определенное время.");
                return repository.findAllByUriAndTimestampAfterAndTimestampBefore(uris, start, end);
            }
        }
    }
}
